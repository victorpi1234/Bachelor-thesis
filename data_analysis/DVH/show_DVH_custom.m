function show_DVH_custom(dvh,cst,varargin)
...
p.addParameter('MaxDose',[],@(x) isempty(x) || (isscalar(x) && x > 0)); % New parameter
...
customMaxDose = p.Results.MaxDose; % Get custom max dose
...
% Modified xlim setting to use custom max dose if provided
if isempty(customMaxDose)
    xlim(axesHandle,[0 1.2*maxDVHdose]);
else
    xlim(axesHandle,[0 customMaxDose]);
end
