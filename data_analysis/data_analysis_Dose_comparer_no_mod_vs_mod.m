function [] = data_analysis_Dose_comparer_no_mod_vs_mod(ct_mod,cst_mod,ct_no_mod, cst_no_mod, resultGUI,resultGUI_no_mod, pln, modulator_type, rows, slice,pics_on_or_off, Doserange, Doserange_Difference)
...
subplot(rows,3,1);
matRad_plotSliceWrapper(gca, ct_no_mod, cst_no_mod, 1, resultGUI_no_mod.physicalDose, 3, slice, [], [], [], ...     % thresh, alpha, contourColorMap
...
title('No modulator dose distribution');

subplot(rows,3,2);
matRad_plotSliceWrapper(gca, ct_mod, cst_mod, 1, resultGUI.physicalDose, 3, slice, [], [], [], ...     % thresh, alpha, contourColorMap
    [], Doserange, [], [], 'Dose [Gy]', false); 
% matRad_plotSliceWrapper(gca,ct_mod,cst_mod,1,resultGUI.physicalDose,3,slice);
%clim([0 5e-3]);
% ylabel(colorbar, 'Dose [Gy]');
title(['Dose distribution with ' modulator_type ' modulator']);

% Subplot 3: Another variation (example)
subplot(rows,3,3);
matRad_plotSliceWrapper(gca, ct_mod, cst_mod, 1, resultGUI.physicalDose- resultGUI_no_mod.physicalDose, 3, slice, [], [], [], ...     % thresh, alpha, contourColorMap
...
% clim([-5e-3 5e-3]);
ylabel(colorbar, 'Dose [Gy]');
title_line1 = ['Dose difference with ' modulator_type ''];
title_line2 = 'modulator vs no modulator';
