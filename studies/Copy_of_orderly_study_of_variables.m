...
for PTV_center = [-50, 0]
    [ct_no_mod, cst_no_mod] = makeCST_water(ct_no_mod, 30, 20, [PTV_center 0 0], [],[],[],-50);
    for boxsize_y = 20 % [2, 4, 8, 14, 20]
        max_height = boxsize_y - 1;
        tooth_heights = unique(round(linspace(max_height, 1, min(4, max_height))), 'stable');
        for tooth_height = tooth_heights %boxsize_y-1:2:1
            for removal_lenght_y = [1,2,3]%[1, 2, 3, 6, 10]
                for removal_space_y = 2*removal_lenght_y % removal_space_y= removal_lenght_y+1:2:removal_lenght_y+6
                    ...
                    for doseGridRes = [1, 3]
                        ...
                        % Save with structure
                        baseFolder = 'C:\Users\victo\bachelor_thesis\dkfz\MymatRad\Project\orderly_study_14_05_data';
