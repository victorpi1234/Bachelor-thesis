function [ct, cst] = delete_Cone_mod(ct, cst, coneBaseRadius, coneHeight, yLocation, baseThickness, boxSize, coneSpacingX_Z,HU, varargin)
        ...
        for z = 1:ct.cubeDim(3)
            if abs(x - Modulator_center(2)) <= halfSize(2) && ...
               abs(z - Modulator_center(3)) <= halfSize(3)
                modulatorHelper(y,x,z) = 0;  % Fill the base
            end
        end
    end
end
                    ...
                    if dx^2 + dz^2 <= currentRadius^2
                        ...
                        if voxelX >= 1 && voxelX <= ct.cubeDim(2) && ...
                           ...
                           voxelZ >= 1 && voxelZ <= ct.cubeDim(3)
                            modulatorHelper(voxelY, voxelX, voxelZ) = 0;
                        end
                    end
                end
            end
