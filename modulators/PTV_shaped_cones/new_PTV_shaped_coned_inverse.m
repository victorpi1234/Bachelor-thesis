function [ct, cst] = new_PTV_shaped_coned_inverse(ct, cst, modulatorDepth, yLocation, coneBaseRadius, coneHeight_on_top_of_mod, coneSpacingX, coneSpacingZ, HU, cutout_start, width_margingsYXZ, structureColor, varargin)
   % Parameters with defaults
    ...
    %% Create PTV cutout from BOTTOM (modified for 180° rotation)
    cutout_start_vox = round(cutout_start/ct.resolution.y);
    bottom_of_modulator = Modulator_center(1) + halfSize(1);
    new_y = bottom_of_modulator - cutout_start_vox - (max(ptv_y) - ptv_y); % Modified for bottom cutout
    ...
    modulatorMask(moved_PTV) = 0;  % Remove PTV from base (now from bottom)
    
    %% Find actual TOP surface (previously bottom) after PTV removal
    top_surface = zeros(ct.cubeDim(2), ct.cubeDim(3));
    for x = 1:ct.cubeDim(2)
        for z = 1:ct.cubeDim(3)
            y_values = find(modulatorMask(:,x,z));
            if ~isempty(y_values)
                top_surface(x,z) = min(y_values); % Lowest y = top in MATLAB coordinates
            end
        end
    end    
    %% Find the highest point of the modulator (minimum y-value)
    min_y = min(find(modulatorMask(:)));  % Find the highest point (minimum y in MATLAB coords)
    [min_y_coord, ~, ~] = ind2sub(ct.cubeDim, min_y);
    target_y = min_y_coord - coneHeight_on_top_of_mod;  % All cones will end at this y-level
    
    %% Create upward-pointing cones ending at target_y
    ...
    for x_idx = 1:length(x_positions)
        for z_idx = 1:length(z_positions)
            ...
            % Calculate actual cone height needed to reach target_y
            actual_cone_height = y_base - target_y;
            if actual_cone_height <= 0, continue; end % Skip if no height needed
            
            % Grow upward (decreasing y) from the actual top
            for y_offset = 0:actual_cone_height
                y = y_base - y_offset; % Moving upward (decreasing y)
                if y < 1, break; end % Stop if we go beyond volume bounds
                
                % Calculate current radius (linearly decreasing)
                currentRadius = coneBaseRadius * (1 - y_offset/actual_cone_height);
