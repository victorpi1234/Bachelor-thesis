function [ct, cst] = PTV_shaped_coned_inverse(ct, cst, modulatorDepth, yLocation, coneBaseRadius, coneHeight, coneSpacingX, coneSpacingZ, HU, cutout_start, width_margingsYXZ, structureColor, varargin)
    ...
    %% Create upward-pointing cones (getting wider) starting from actual top surface
    ...
    for x_idx = 1:length(x_positions)
        for z_idx = 1:length(z_positions)
            ...
            for y_offset = 0:coneHeight
                ...
                currentRadius = coneBaseRadius * (1 - y_offset/coneHeight); % Radius decreases as we go up
