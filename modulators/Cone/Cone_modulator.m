function [ct, cst] = Cone_modulator(ct, cst, coneBaseRadius, coneHeight, yLocation, boxSize, coneSpacingX_Z, HU, varargin)
...
Modulator_center = [(center(1) - yLocation), center(2), center(3)]; % Center of the modulator (center(1)-110) for original dimension

% Create a modulator box around the PTV center
modulatorHelper = zeros(ct.cubeDim);
halfSize = boxSize / 2;
%%
% Loop to create conical teeth
for x = Modulator_center(2) - halfSize(2):coneSpacingX:Modulator_center(2) + halfSize(2)
    for z = Modulator_center(3) - halfSize(3):coneSpacingZ:Modulator_center(3) + halfSize(3)
        ...
        for y = Modulator_center(1):Modulator_center(1) + coneHeight
            % Calculate the current radius of the cone (tapered)
            currentRadius = coneBaseRadius * (1 - (y - Modulator_center(1)) / coneHeight);
