function [ct, cst] = Cones_shaped_like_PTV(ct, cst, modulatorDepth, yLocation, coneBaseRadius, maxConeHeight, coneSpacingX, coneSpacingZ, HU, cutout_start, width_margingsYXZ, structureColor, varargin)
    ...
    %% Create normalized PTV depth map (1 = deepest, 0 = shallowest)
    ...
    %% Create cones with DIRECT relationship to PTV depth
    ...
    for x_idx = 1:length(x_positions)
        for z_idx = 1:length(z_positions)
            ...
            x = max(1, min(x, ct.cubeDim(2)));
            z = max(1, min(z, ct.cubeDim(3)));
            
            y_base = top_surface(x,z);
            if y_base == 0, continue; end
            
            % DIRECT relationship: longer cones where PTV is deeper
            if x >= 1 && x <= size(PTV_depth_map_norm,1) && z >= 1 && z <= size(PTV_depth_map_norm,2)
                cone_length = maxConeHeight * PTV_depth_map_norm(x,z); % Removed the 1- operation
            else
